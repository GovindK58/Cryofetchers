...
bool CACHE::filllike_miss(std::size_t set, std::size_t way, PACKET& handle_pkt)
{
  ...
  BLOCK* to_evict = &block[set * NUM_WAY + way];
  BLOCK* h_lvl = nullptr;
  BLOCK* hh_lvl = nullptr;
  CACHE* high_high = nullptr;

  if(higher_level){
    // first higher level

    uint32_t h_set = higher_level->get_set(to_evict->address);
    uint32_t h_way = higher_level->get_way(to_evict->address, h_set);

    if(h_way == higher_level->NUM_WAY){
      // not present in 1st higher level, so do nothing
    }
    else{
      h_lvl = &higher_level->block[h_set * higher_level->NUM_WAY + h_way];
      if (h_lvl->dirty){
        // since block is dirty at l1d, it wiil be evicted
        to_evict = h_lvl;
      }

      high_high = higher_level->higher_level;

      if(high_high){
        // 2nd higher level

        uint32_t h_set = high_high->get_set(to_evict->address);
        uint32_t h_way = high_high->get_way(to_evict->address, h_set);

        if(h_way == high_high->NUM_WAY){
          // not present in 2nd higher level, so do nothing
        }
        else{
          hh_lvl = &high_high->block[h_set * high_high->NUM_WAY + h_way];
          if (hh_lvl->dirty){
            // since block is dirty at 2nd higher level, it wiil be evicted
            to_evict = hh_lvl;
          }
        }
      }
  ...
  bool evicting_dirty = (lower_level != NULL) && to_evict->dirty;
  uint64_t evicting_address = 0;

  // no bypass for inclusive cache
    if (evicting_dirty) {
      ...
      writeback_packet.data = to_evict->data;
      ...
      if(higher_level)
        higher_level->invalidate_entry(to_evict->address);
      if(high_high)
        high_high->invalidate_entry(to_evict->address);
    }
